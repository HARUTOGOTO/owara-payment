// LIFF → /checkout ブリッジ固定版（prod）
// config は ./config.js を ?cb= 付きで確実に最新を読む
(() => {
  ...
  (async () => {
    // 1) config.js を読む（★ v2用の config.v2.js ではなく prod の config.js）
    let cfg;
    try {
    ...
    try {
      ...
      // 申込 → /checkout ブリッジ（/checkout→UnivaPay は location.href で履歴を残す）
